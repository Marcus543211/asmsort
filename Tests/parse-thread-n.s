...
parse_loop_end:
    ...
    movq $247, %rax # waitid syscall
    movq $0, %rdi # P_ALL (wait for any thread)
    movq $0, %rsi # Pid (ignored as we wait for any)
    movq $0, %rdx # infop (don't care)
    movq $0x80000004, %r10 # WEXITED + __WCLONE
    movq $0, %r8 # rusage (don't care)
    syscall
