...
_start:
    ...
    movq %r15, %r8 # File pointer
    leaq (%r15, %r13), %r9 # File end
    movq %r14, %r10 # Result pointer

.macro parse_digit
    movzbq (%r8), %rbx # Read the next byte (zero extending)
    addq $1, %r8 # Increase counter
...
.align 16
start_of_number:
    movzbq (%r8), %rax # Next digit into %rax, clearing it
    addq $1, %r8
    ...
    addq $1, %r8 # Count the \t or \n
    # Normally when jumping it has been counted
end_of_number:
    movw %ax, (%r10)
    addq $2, %r10 # Move the pointer
